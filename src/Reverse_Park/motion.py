...
class MotionController(Node):
    ...
    def __init__(self):
        ...
        # 控制变量
        self.current_mode = 0       # 当前模式 1 = 低头后退 2 = 左转 3 = 右移
        self.progress = 0
    ...
    def execute_down_back_walk(self):
        cmd = robot_control_cmd_lcmt()
        cmd.mode = 11               # 行走模式
        cmd.gait_id = 27             # 抬头后退行走步态
        cmd.contact = 0
        cmd.life_count = 1
        cmd.pos_des = [0.0, 0.0, 0.0]
        cmd.rpy_des = [0.0, -0.5, 0.0]  # 抬头角度
        ...
        cmd.duration = 0
        self.robot_ctrl.Send_cmd(cmd)
        self.get_logger().info("执行低头后退行走命令")
    ...
    def motion_control_loop(self):
        with self.mode_lock:
            ...
            if self.current_mode == 0:
                ...
                #self.execute_head_down_walk()
                self.progress = 100
                self.get_logger().info(f"当前模式是低头后退的进度:{self.progress}")
                ...
                self.progress = 100
                #self.get_logger().info(f"当前模式是低头转弯的进度:{self.progress}")
