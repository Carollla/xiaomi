    ...
    def robot_status_callback(self, msg):
        ...
        if len(msg.data) >= 2:
            ...
            if current_mode == 0 :
                ...
                if self.right_count == 0 and abs(self.turned_angles()-225) <= 1:
                    self.right_count += 1
                    self.state = "RIGHT_ROUND_WALK"
                    ...
                    self.get_logger().info("当前角度为：%s" % self.current_angle)                    
                    self._right_round_walk_cmd_sent = False
                elif self.right_count == 1 and abs(self.turned_angles()-215) <= 1:
                    self.right_count += 1
                ...
                if self.left_count == 0 and abs(self.turned_angles()-265) <= 1:
                    self.left_count += 1
                    ...
                    self._left_round_walk_cmd_sent = False
                elif self.left_count == 1 and abs(self.turned_angles()-110) <= 1:
                    self.get_logger().info("[状态变更] 检测到右圆形走完成，下面进左转步态")
                    self.current_angle = normalize_angle(get_current_yaw(self.imu_provider))
                    self.turned_angle = 0
                    self.turn_mode = 0
                    self.get_logger().info("当前角度为：%s" % self.current_angle)
